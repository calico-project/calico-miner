use crate::{
    miner::MinerManager,
    proto::{
        calicod_message::Payload, rpc_client::RpcClient, CalicodMessage, GetBlockTemplateRequestMessage,
...
#[allow(dead_code)]
pub struct CalicodHandler {
    client: RpcClient<TonicChannel>,
    pub send_channel: Sender<CalicodMessage>,
    stream: Streaming<CalicodMessage>,
...
impl CalicodHandler {
    ...
    pub async fn client_send(&self, msg: impl Into<CalicodMessage>) -> Result<(), SendError<CalicodMessage>> {
        self.send_channel.send(msg.into()).await
    }

    pub async fn client_get_block_template(&mut self) -> Result<(), SendError<CalicodMessage>> {
    ...
    pub async fn listen(&mut self, miner: &mut MinerManager, shutdown: ShutdownHandler) -> Result<(), Error> {
        while let Some(msg) = self.stream.message().await? {
            ...
            match msg.payload {
                ...
                None => warn!("calicod message payload is empty"),
            }
        }
        Ok(())
    }
    ...
    async fn handle_message(&mut self, msg: Payload, miner: &mut MinerManager) -> Result<(), Error> {
        match msg {
            ...
            Payload::GetInfoResponse(info) => info!("Calicod version: {}", info.server_version),
