...
pub struct MinerManager {
    ...
    send_channel: Sender<CalicodMessage>,
...
impl MinerManager {
    ...
    pub fn process_block(&mut self, block: Option<RpcBlock>) -> Result<(), Error> {
        let state = if let Some(b) = block {
            ...
            warn!("Calicod is not synced, skipping current template");
            None
        };
    ...
    pub fn launch_cpu_miner(
        ...
        #[cold]
        fn found_block(send_channel: &Sender<CalicodMessage>, block: RpcBlock) -> Result<(), Error> {
            let block_hash = block.block_hash().expect("We just got it from the state, we should be able to hash it");
            send_channel.blocking_send(CalicodMessage::submit_block(block))?;
    ...
    async fn log_hashrate(hashes_tried: Arc<AtomicU64>) {
        ...
        for i in 0u64.. {
            ...
            if hashes == 0 && i % 2 == 0 {
                warn!("Calicod is still not synced");
            } else if hashes != 0 {
