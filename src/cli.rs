...
pub struct Opt {
    ...
    #[clap(short = 's', long = "calicod-address", default_value = "127.0.0.1", display_order = 1)]
    /// The IP of the calicod instance
    pub calicod_address: String,
...
impl Opt {
    pub fn process(&mut self) -> Result<(), Error> {
        if self.calicod_address.is_empty() {
            self.calicod_address = "127.0.0.1".to_string();
        }

        if !self.calicod_address.starts_with("grpc://") {
            IpAddr::from_str(&self.calicod_address)?;
            let port = self.port();
            self.calicod_address = format!("grpc://{}:{}", self.calicod_address, port);
        }
        log::info!("Calicod address: {}", self.calicod_address);
