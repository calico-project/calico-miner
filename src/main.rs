...
use crate::{
    cli::Opt, client::CalicodHandler, miner::MinerManager, proto::NotifyNewBlockTemplateRequestMessage, target::Uint256,
};

mod calicod_messages;
mod cli;
mod client;
mod miner;
mod pow;
...
async fn main() -> Result<(), Error> {
    ...
    while !shutdown.is_shutdown() {
        match CalicodHandler::connect(opt.calicod_address.clone(), opt.mining_address.clone(), opt.mine_when_not_synced)
            .await
        {
            Ok(mut client) => {
                ...
                if let Err(e) = client.listen(&mut miner_manager, shutdown.clone()).await {
                    warn!("Disconnected from calicod: {}. Retrying", e);
                }
            }
            Err(e) => {
                warn!("Failed to connect to calicod: {}. Retrying in 10 seconds...", e);
            }
        }
